...
import evaluation
from get_spearman import get_spearman_edges
...
vector_path = 'output/w2v/wv.wordvectors'
...
corpus_path_w5 = 'input/Brown/brown.csv'
...
def get_w2v(corpus_path, vector_path, vocab_path, mapped_concepts_path, edges_path, gml_path, window_size = 2, google_vectors=False, overwrite_model = True, overwrite_gml=True):

    if overwrite_model==True or not Path(model_path).is_file():
        print("building w2v model...")
        word2vecmodel.build_model(corpus_path, vector_path, vocab_path, window_size=window_size, google_vectors=google_vectors)
...
def get_highest_weight(models, path):
    ...
    for model in models:
        ...
        edge_IDs = {graph.vs[id1]["Gloss"] + " : " + graph.vs[id2]["Gloss"]: (weights[i], graph.vs[id1]["wordtypes"], graph[id2]["wordtypes"]) for i, (id1, id2) in enumerate(edgelist)}
        ...
        wordtypes2 = []
        for pair in top_10:
            w, wt1, wt2 = edge_IDs[pair]
            top_10_weights.append(pair + ": " + str(w))
            wordtypes1.append(wt1)
            wordtypes2.append(wt2)
        ...
        #print(top_10_str)
        weight_dic = {model["name"]: top_10s[model["name"]] for model in models}
    df = pd.DataFrame(weight_dic)
    df.to_csv(path)
    return(top10s)
...
if __name__=="__main__":
    ...
    get_w2v(corpus_path, model_path, vocab_path, mapped_concepts_path, edges_path, w2v_gml_path, window_size = 2, overwrite_model=overwrite_model, overwrite_gml=overwrite_w2v)
    #create w2v network w5
    get_w2v(corpus_path, model_path_w5, vocab_path, mapped_concepts_path, edges_path_w5, w2v_gml_path_w5, window_size = 5, overwrite_model=overwrite_model_w5, overwrite_gml=overwrite_w2v_w5)
    ...
    # compare to pairs in MultiSimLex
    '''
    SimLex_scores = []
    for model in models:
        if model["compare"]:
            ...
            SimLex_scores.append((model["name"], spearman_SimLex, edges_compared))
    p_values = []


    SimLex_df = pd.DataFrame({"network": [name for (name, score, edge) in SimLex_scores],
                              "edges compared": [edge for (name, score, edge) in SimLex_scores],
                              "Spearman Rank Coefficient": [round(score[0],4) for (name, score, edge) in SimLex_scores],
                              "p-value": [round(score[1],4) for (name, score, edge) in SimLex_scores]})
    SimLex_df.to_csv(SimLex_csv_path, header="Spearman Correlation to MultiSimLex similarity scores")
    print("saved comparison to MultiSimLex scores to", SimLex_csv_path)
    print("*** MULTISIMLEX SCORES ***\n", SimLex_scores)
    '''
    ...
    print(get_highest_weight(models[0:3], top_10_path))